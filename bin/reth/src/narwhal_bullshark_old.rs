...
use alloy_primitives::{TxHash, B256, Address, U256, Bytes};
use tokio::sync::mpsc;
    ...
    EvmConfig: ConfigureEvm<Primitives = reth_ethereum_primitives::EthPrimitives> + Clone + Send + Sync + 'static,
{
    setup_mempool_integration_with_optional_engine(
        bridge, transaction_pool, provider, evm_config, task_executor, None
    )
}

/// Set up mempool integration with Narwhal + Bullshark with optional engine handle
pub fn setup_mempool_integration_with_optional_engine<Pool, N, EvmConfig>(
    mut bridge: NarwhalRethBridge,
    transaction_pool: Arc<Pool>,
    provider: BlockchainProvider<N>,
    evm_config: EvmConfig,
    task_executor: TaskExecutor,
    engine_handle: Option<reth_engine_primitives::BeaconConsensusEngineHandle<reth_node_ethereum::EthEngineTypes>>,
) -> eyre::Result<tokio::task::JoinHandle<()>>
where
    Pool: TransactionPool<Transaction = EthPooledTransaction> + TransactionPoolExt + Send + Sync + 'static,
    N: ProviderNodeTypes<Primitives = reth_ethereum_primitives::EthPrimitives> + 'static,
    EvmConfig: ConfigureEvm<Primitives = reth_ethereum_primitives::EthPrimitives> + Clone + Send + Sync + 'static,
{
    ...
    let handle = task_executor.spawn(async move {
        ...
        // Process finalized batches from consensus
        // 
        // Note: With the new payload builder approach, we no longer manually create blocks
        // and submit them via engine.new_payload(). Instead, finalized batches are forwarded
        // to the custom payload builder, which integrates with Reth's execution pipeline.
        // The engine API will naturally request payloads when needed for block building.
        let mut batch_count = 0u64;
        loop {
            // Check for new finalized blocks with a timeout
            // TODO: Once payload builder integration is complete, switch back to get_next_finalized_batch
            ...
            match timeout {
                Ok(Ok(Some(sealed_block))) => {
                    ...
                    // Now that hash mismatch is fixed, submit the block via engine API
                    if let Some(ref engine) = engine_handle {
                        // Convert to the engine's expected payload type
                        use reth_node_ethereum::EthEngineTypes;
                        use reth_payload_primitives::PayloadTypes;
                        let payload = <EthEngineTypes as PayloadTypes>::block_to_payload(sealed_block.clone());
                        
                        info!(target: "reth::narwhal_bullshark", 
                              "Submitting block #{} to engine API (hash: {})",
                              sealed_block.number, sealed_block.hash());
                        
                        // Submit the payload to the engine
                            ...
                            Ok(payload_status) => {
                                ...
                                match payload_status.status {
                                    PayloadStatusEnum::Valid => {
                                        info!(target: "reth::narwhal_bullshark", 
                                              "✅ Block #{} accepted by engine as valid",
                                              sealed_block.number);
                                        
                                        // Now update fork choice to make it canonical
                                        use alloy_rpc_types::engine::ForkchoiceState;
                                        ...
                                        match engine.fork_choice_updated(state, None, reth_node_api::EngineApiMessageVersion::default()).await {
                                            Ok(response) => {
                                                match response.payload_status.status {
                                                    PayloadStatusEnum::Valid => {
                                                        info!(target: "reth::narwhal_bullshark", 
                                                              "✅ Block #{} made canonical via engine API",
                                                              sealed_block.number);
                                                        
                                                        // Update consensus bridge chain state
                                                        bridge.update_chain_state(sealed_block.number, sealed_block.hash()).await;
                                                    }
                                                    _ => {
                                                        error!(target: "reth::narwhal_bullshark", 
                                                               "❌ Fork choice update failed for block #{}: {:?}",
                                                               sealed_block.number, response.payload_status);
                                                    }
                                                }
                                            }
                                            Err(e) => {
                                                error!(target: "reth::narwhal_bullshark", 
                                                       "❌ Failed to update fork choice for block #{}: {}",
                                    ...
                                    PayloadStatusEnum::Invalid { validation_error } => {
                                        error!(target: "reth::narwhal_bullshark", 
                                               "❌ Block #{} rejected by engine as invalid: {}",
                                               sealed_block.number, validation_error);
                                    }
                                    _ => {
                                        error!(target: "reth::narwhal_bullshark", 
                                               "❌ Unexpected payload status for block #{}: {:?}",
                                               sealed_block.number, payload_status);
                                    }
                                }
                            }
                            Err(e) => {
                                error!(target: "reth::narwhal_bullshark", 
                                       "❌ Failed to submit block #{} to engine: {}",
                                       sealed_block.number, e);
                            }
                        }
                    } else {
                        warn!(target: "reth::narwhal_bullshark", 
                              "No engine handle available, cannot submit block #{}", 
                              sealed_block.number);
                    }
                }
                Ok(Ok(None)) => {
        ...
        info!(target: "reth::narwhal_bullshark", 
              "Consensus bridge task ending after processing {} blocks", batch_count);
    });
