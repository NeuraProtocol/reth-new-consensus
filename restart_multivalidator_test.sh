...
USE_REAL_CONSENSUS=true $RETH_BINARY node \
  ...
  --narwhal.max-batch-delay-ms 200 \
  --narwhal.max-batch-size 100000 \
  --narwhal.gc-depth 5000 \
  --bullshark.min-block-time-ms 200 \
  --narwhal.cache-size 10000 \
  --narwhal.max-concurrent-requests 200 \
  --narwhal.connection-timeout-ms 300000 \
  --narwhal.request-timeout-ms 600000 \
  --narwhal.retry-attempts 20 \
  --narwhal.retry-base-delay-ms 500 \
  --narwhal.retry-max-delay-ms 30000 \
  --narwhal.sync-retry-delay-ms 10000 \
  --narwhal.certificate-buffer-size 10000 \
  --narwhal.max-transactions-per-batch 100 \
  --narwhal.batch-creation-interval-ms 200 \
