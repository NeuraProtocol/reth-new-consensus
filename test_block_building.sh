#!/bin/bash
# Test script to verify block building with correct state roots

set -e

echo "Testing Narwhal+Bullshark block building..."

# Navigate to the project directory
cd /srv/tank/src/reth-new-consensus

# Build the test binary
echo "Building test binary..."
cargo build --release --example test-narwhal-block-builder 2>/dev/null || {
    echo "Creating test example..."
    
    # Create a test example
    cat > examples/test-narwhal-block-builder.rs << 'EOF'
...
EOF
    ...
    # Build the example
    cargo build --release --example test-narwhal-block-builder
}

# Run the test
echo "Running block building test..."
./target/release/examples/test-narwhal-block-builder
